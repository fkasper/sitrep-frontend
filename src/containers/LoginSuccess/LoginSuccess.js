...
@connect(
    state => ({
      ...
      exercises: (state.exercises.data ? state.exercises.data.Exercises : {})
    }),
    ...
    return (user && <div style={{flex: 1}}>
      <Helmet title="Select an exercise" />
            ...
            <div className={styles.card + ' ' + styles.card2} style={{position: 'relative'}}>
              ...
              { (exercises && exercises.length) ? exercises.map((name) => {
                return (
                  <a className={styles.option} onClick={this.storeExId.bind(this, name)}>
                    ...
                    <div className={styles.info}>
                      <div className={styles.name}>There are no active exercises for you</div>
                    </div>
                  </div>
              }
